...
if __name__ == "__main__":
    ###############################################################################
    # 0. 参数设置
    jpegimages_dir = "./JPEGImages"  # 图片保存位置
    annotations_dir = "./Annotations"  # 标注文件保存位置

    # 分为3份：train,val,test
    trainval_percent = 0.8  # (train+val)/(train+val+test)
    train_percent = 0.8  # (train)/(train+val)
  ...
  #修改为自己的类别,多个类["class1","class2"]
    ...
    dir2_train = "./labels/train0712"
    dir2_val = "./labels/val0712"
    dir2_test = "./labels/test0712"
    ...
    print("=" * 5, "/t1. checking jpg and xml/t", "=" * 5)
    checkJpgXml(jpeg_dir=jpegimages_dir, annot_dir=annotations_dir)

    # 2. 按照比例创建训练、验证、测试
    print("=" * 5, "/t2. split train, val, test/t", "=" * 5)
    create_main_txts(trainval_percent=trainval_percent,
                     train_percent=train_percent)

    # 3. 将坐标进行归一化，生成labels中的txt文件
    print("=" * 5, "/t3. create txt files in labels/t", "=" * 5)
    gen_voc_lable(classes)

    # 4. 构建pytorch版本yolov3格式
    print("=" * 5, "/t4. create yolov3 format/t", "=" * 5)
    make_for_torch_yolov3(jpegimages_dir, dir_label,
                          dir1_train, dir1_val, dir1_test,
                          dir2_train, dir2_val, dir2_test,
                          main_trainval, main_train, main_val, main_test)

    # 5. 收尾，将中间文件删除
    print("=" * 5, "/t5. delete temporary directory/t", "=" * 5)
    ...
    print("之后的操作：")
    print("1. 将images,labels文件夹放到yolov3里的data/VOC0712/文件夹中")
    print("2. 将2007_train.txt 2007_test.txt文件放到yolov3里的data/VOC0712/文件夹中")
